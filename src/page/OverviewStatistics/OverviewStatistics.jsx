...
import { Button, Statistic, Input, Col, Row, Select, Modal, Card, Tooltip } from 'antd';
import { UserOutlined, SettingOutlined, SmileOutlined, SyncOutlined, SearchOutlined, ConsoleSqlOutlined, DollarOutlined, FileSearchOutlined } from '@ant-design/icons';
import { APIgetAllStatistics, APIgetStatisticsAcount, APIgetStatisticsAcountDetail, APIgetStatisticsRequest, APIgetStatisticsRequestDetail, APIgetStatisticsRevenue, APIgetStatisticsSession, APIgetStatisticsSessionDetail } from "../../api/api";
const currentYear = new Date().getFullYear();

export default function OverviewStatistics() {
  const [statisticAll, setStatisticAll] = useState();
  const [labelChart, setLabelChart] = useState("");
  const [labelChartPie, setLabelChartPie] = useState("");
  ...
  const arrayData = [
    ...
    { lable: "Request", handle: (year) => { return APIgetStatisticsRequest(year) }, handlePie: () => { return APIgetStatisticsRequestDetail() } },   
    { lable: "Acount", handle: (year) => { return APIgetStatisticsAcount(year) }, handlePie: () => { return APIgetStatisticsAcountDetail() } }  
  ];
  ...
  const options = {
    ...
    const dataPie = {
    labels: arrayDataDetail.map((item => item.label)),
    datasets: [
      {
        label: "quantity: ",
  ...
  const handleOnclickViewPie = async (lable) => {
    setLabelChartPie(lable)
    try {
      const itemLable = arrayData.find(item => item.lable == lable)
      itemLable.handlePie().then((rs) => {
        console.log(rs.data)
  ...
  return (
    <>
      ...
      <Row justify="center" gutter={20}>
        ...
        <Col span={6}>
          ...
          >
            <Tooltip title="Session Detail Pie Chart">
              <SettingOutlined style={iconStyle} onClick={()=>{handleOnclickViewPie("Session")}}/>
            </Tooltip>
        ...
        <Col span={6}>
          ...
          >
            <Tooltip title="Request Detail Pie Chart">
              <FileSearchOutlined style={iconStyle} onClick={()=>{handleOnclickViewPie("Request")}} />
            </Tooltip>
        ...
        <Col span={6}>
          ...
          >
            <Tooltip title="Member Detail Pie Chart">
              <UserOutlined style={iconStyle} onClick={()=>{handleOnclickViewPie("Acount")}}/>
            </Tooltip>         
    ...
    > 
      <h2>{"Detail Statistic "+labelChartPie}</h2>    
